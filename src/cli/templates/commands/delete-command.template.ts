...
export function deleteCommandTemplate(entity: string): string {
    return `
        ...
        export class Delete${stringCapitalize(entity)}Command implements Command<string, ${stringCapitalize(entity)}> {
            ...
            async execute(id: string): Promise<${stringCapitalize(entity)}> {
                try {
                    const deleted${stringCapitalize(entity)} = this.fileRepository.delete(id);
