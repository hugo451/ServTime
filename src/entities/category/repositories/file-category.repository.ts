...
export class FileCategoryRepository extends Repository<Category> {
    private static _path = path.join(
        __dirname,
    ...
    private static _readFile(): Category[] {
        try {
            ...
            throw new CategoryCreateException(
                'Failed to read file.',
                CategoryCreateErrorCode.FILE_READ_ERROR,
            );
        }
    }
    ...
    private static _writeFile(categorys: Category[]): void {
        try {
            fs.writeFileSync(
                FileCategoryRepository._path,
                JSON.stringify(categorys, null, 2),
            );
        } catch (error) {
            throw new CategoryCreateException(
                'Failed to write file.',
                CategoryCreateErrorCode.FILE_WRITE_ERROR,
            );
        }
    }
