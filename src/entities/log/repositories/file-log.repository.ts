...
export class FileLogRepository extends Repository<Log> {
    ...
    private static _readFile(): Log[] {
        try {
            ...
            throw new LogCreateException('Failed to read file.', LogCreateErrorCode.FILE_READ_ERROR);
        }
    }

    private static _writeFile(logs: Log[]): void {
        try {
            fs.writeFileSync(FileLogRepository._path, JSON.stringify(logs, null, 2));
        } catch (error) {
            throw new LogCreateException('Failed to write file.', LogCreateErrorCode.FILE_WRITE_ERROR);
        }
    }
    ...
    delete(id: string): Log {
        try {
            ...
            if (index === -1) {
                throw new LogCreateException('Log not found.', LogCreateErrorCode.NOT_FOUND);
            }
            const [deletedLog] = logs.splice(index, 1);
            FileLogRepository._writeFile(logs);
            return deletedLog;
        } catch (error) {
            throw new LogCreateException('Failed to delete log.', LogCreateErrorCode.DELETE_FAILED);
        }
    }
    ...
    findAll(): Log[] {
        try {
            ...
            throw new LogCreateException('Failed to find logs.', LogCreateErrorCode.FETCH_FAILED);
        }
    }
    ...
    update(id: string, body: Log): Log {
        try {
            ...
            throw new LogCreateException('Failed to update log.', LogCreateErrorCode.UPDATE_FAILED);
        }
    }
}
